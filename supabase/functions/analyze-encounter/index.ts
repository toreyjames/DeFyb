...
const analyzeEncounterNote = (noteText: string, billedCode = "99213") => {
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: inserted, error: insertError } = await userClient
      ...
      .select(
        "id, specialty, billed_code, suggested_code, confidence, rationale, gaps, suggestions, estimated_delta_per_visit, estimated_monthly_recovery, note_snippet, created_at",
      )
      .single();
